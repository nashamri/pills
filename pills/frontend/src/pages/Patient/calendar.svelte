              ...
              <ul class="day-doses">
                {#each day.doses as d}
                  <li class="day-dose {d.status}">
                    ...
                    {#if d.status === 'pending' || (day.isToday && d.isPast && d.status !== 'taken')}
                      <button class="btn-mini" on:click={() => markAsTaken(d.id)}>✓</button>
                    {/if}
                  </li>
                {/each}
              </ul>
        ...
        {#each monthCells as cell}
          ...
          {:else}
            <div class="cal-cell {cell.isToday ? 'today' : ''} {cell.total > 0 ? (cell.missed > 0 ? 'has-missed' : 'has-taken') : ''}">
              ...
              {#if cell.total > 0}
                <div class="cal-dots">
                  {#each { length: Math.min(cell.taken, 4) } as _}
                    <span class="cdot cdot-taken"></span>
                  {/each}
                  {#each { length: Math.min(cell.missed, 4) } as _}
